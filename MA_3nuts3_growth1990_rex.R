...
A_fit1 <- df %>% filter(year>1990) %>% nest_by(year) %>%
  mutate(mod = list(lm(GDPpc_delta1990 ~ trade_center +
                         nuts_1 +
...
B_fit1 <- df %>% filter(year>1990) %>% nest_by(year) %>%
  mutate(mod = list(lm(GDPpc_delta1990 ~ ln_tc_dist +
                         nuts_1 +
                         latitude + longitude + altitude +
                         lndist_border + lndist_coast + lndist_river + neighborgdp + tradeneighbor + tradeneighbor2 + tradeneighbor3 + tradeneighbor4,
                       data = data)))
...
C_fit1 <- df %>% filter(year>1990) %>% nest_by(year) %>%
  mutate(mod = list(lm(GDPpc_delta1990 ~ trade_years +
                         nuts_1 +
                         latitude + longitude + altitude +
                         lndist_border + lndist_coast + lndist_river + neighborgdp + tradeneighbor + tradeneighbor2 + tradeneighbor3 + tradeneighbor4,
                       data = data)))
...
# PANEL A: trade center dummy
A_fit2 <- df %>% filter(year>1990) %>% nest_by(year) %>%
  mutate(mod = list(lm(GDPpc_delta1990 ~ trade_center +
                         nuts_1 +
                         latitude + longitude + altitude +
...
# PANEL B: distance to trade 
B_fit2 <- df %>% filter(year>1990) %>% nest_by(year) %>%
  mutate(mod = list(lm(GDPpc_delta1990 ~ ln_tc_dist +
                         nuts_1 +
                         latitude + longitude + altitude +
                         capital + mountain + mining + lnarea + lnagri,
...
# PANEL C: centuries of trade
C_fit2 <- df %>% filter(year>1990) %>% nest_by(year) %>%
  mutate(mod = list(lm(GDPpc_delta1990 ~ trade_years +
                         nuts_1 +
                         latitude + longitude + altitude +
                         capital + mountain + mining + lnarea + lnagri,
...
# PANEL A: trade center dummy
A_fit3 <- df %>% filter(year>1990) %>% nest_by(year) %>%
  mutate(mod = list(lm(GDPpc_delta1990 ~ trade_center +
                         nuts_1 +
                         latitude + longitude + altitude +
...
# PANEL B: distance to trade 
B_fit3 <- df %>% filter(year>1990) %>% nest_by(year) %>%
  mutate(mod = list(lm(GDPpc_delta1990 ~ ln_tc_dist +
                         nuts_1 +
                         latitude + longitude + altitude +
                         university_1500 + printingpress_pre1500 + bishop + imperial + hanse + residence + imperialroad + lndist_roman,
...
# PANEL C: centuries of trade
C_fit3 <- df %>% filter(year>1990) %>% nest_by(year) %>%
  mutate(mod = list(lm(GDPpc_delta1990 ~ trade_years +
                         nuts_1 +
                         latitude + longitude + altitude +
                         university_1500 + printingpress_pre1500 + bishop + imperial + hanse + residence + imperialroad + lndist_roman,
...
# PANEL A: trade center dummy
A_fit4 <- df %>% filter(year>1990) %>% nest_by(year) %>%
  mutate(mod = list(lm(GDPpc_delta1990 ~ trade_center +
                         nuts_1 +
                         latitude + longitude + altitude +
...
# PANEL B: distance to trade 
B_fit4 <- df %>% filter(year>1990) %>% nest_by(year) %>%
  mutate(mod = list(lm(GDPpc_delta1990 ~ ln_tc_dist +
                         nuts_1 +
                         latitude + longitude + altitude +
                         eqi100 + inequality2 + lnemp_comp,
...
# PANEL C: centuries of trade
C_fit4 <- df %>% filter(year>1990) %>% nest_by(year) %>%
  mutate(mod = list(lm(GDPpc_delta1990 ~ trade_years +
                         nuts_1 +
                         latitude + longitude + altitude +
                         eqi100 + inequality2 + lnemp_comp,
...
# PANEL A: trade center dummy
A_fit5 <- df %>% filter(year>1990) %>% nest_by(year) %>%
  mutate(mod = list(lm(GDPpc_delta1990 ~ trade_center +
                         nuts_1 +
                         latitude + longitude + altitude +
                         lndist_border + lndist_coast + lndist_river + neighborgdp + tradeneighbor + tradeneighbor2 + tradeneighbor3 + tradeneighbor4 +
...
# PANEL B: distance to trade 
B_fit5 <- df %>% filter(year>1990) %>% nest_by(year) %>%
  mutate(mod = list(lm(GDPpc_delta1990 ~ ln_tc_dist +
                         nuts_1 +
                         latitude + longitude + altitude +
                         lndist_border + lndist_coast + lndist_river + neighborgdp + tradeneighbor + tradeneighbor2 + tradeneighbor3 + tradeneighbor4 +
...
# PANEL C: centuries of trade
C_fit5 <- df %>% filter(year>1990) %>% nest_by(year) %>%
  mutate(mod = list(lm(GDPpc_delta1990 ~ trade_years +
                         nuts_1 +
                         latitude + longitude + altitude +
                         lndist_border + lndist_coast + lndist_river + neighborgdp + tradeneighbor + tradeneighbor2 + tradeneighbor3 + tradeneighbor4 +
...
# PANEL A: trade center dummy
A_fit6 <- df %>% filter(year>1990) %>% nest_by(year) %>%
  mutate(mod = list(lm(GDPpc_delta1990 ~ trade_center +
                         nuts_2 +
                         latitude + longitude + altitude +
                         lndist_border + lndist_coast + lndist_river + neighborgdp + tradeneighbor + tradeneighbor2 + tradeneighbor3 + tradeneighbor4 +
...
# PANEL B: distance to trade 
B_fit6 <- df %>% filter(year>1990) %>% nest_by(year) %>%
  mutate(mod = list(lm(GDPpc_delta1990 ~ ln_tc_dist +
                         nuts_2 +
                         latitude + longitude + altitude +
                         lndist_border + lndist_coast + lndist_river + neighborgdp + tradeneighbor + tradeneighbor2 + tradeneighbor3 + tradeneighbor4 +
...
# PANEL C: centuries of trade
C_fit6 <- df %>% filter(year>1990) %>% nest_by(year) %>%
  mutate(mod = list(lm(GDPpc_delta1990 ~ trade_years +
                         nuts_2 +
                         latitude + longitude + altitude +
                         lndist_border + lndist_coast + lndist_river + neighborgdp + tradeneighbor + tradeneighbor2 + tradeneighbor3 + tradeneighbor4 +
