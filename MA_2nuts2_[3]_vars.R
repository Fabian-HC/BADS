...
df <- read.csv('data_nuts2_long.csv')
...
## tc density: number of NUTS3 trade centers per 1000km^2 area of NUTS2 region
df["tc_dens"] <- (df$tc_abs*1000)/df$AREA
df["tc_dens_1"] <- (df$tc_abs_1*1000)/df$AREA
df["tc_dens_2"] <- (df$tc_abs_2*1000)/df$AREA
...
# === save extended long form df to directory === #
write.csv(df, file="MA_nuts2_data.csv")
