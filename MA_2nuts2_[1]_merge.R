...
#load raw data
wolfV6 <- read.csv2('data_roseswolf_v6.csv', header = TRUE)
...
wahl_agg<-group_by(df_wahl, nuts_2) %>%
  summarise(
    ...
    tc_ratio = sum(trade_city_final)/n(),
    tc_ratio_1 = sum(trade_city_final_4)/n(),
    tc_ratio_2 = sum(trade_city_alt)/n(),
    tc_abs = sum(trade_city_final),
    tc_abs_1 = sum(trade_city_final_4),
    tc_abs_2 = sum(trade_city_alt),
    ...
    tc_dist = cust_dist(exp(lndist_trade_city1)),
...
#make sure that vars "trade_center" and "trade_years" (i.e. centuries of trade) have the same number of non-zero entries
sum(df_merge$trade_center != 0)
sum(df_merge$trade_years != 0)

#check that vars "trade_center" and "trade_years" are available in pairs. [expect 104 TRUE statements]
for(i in 1:nrow(df_merge)){
  a=df_merge[i,which(colnames(df_merge)=="trade_center")]
  b=df_merge[i,which(colnames(df_merge)=="trade_years")]
  if(a!=0 & b==0 | a==0 & b!=0){ 
    cat(i,"FALSE","\n")}
  else {
    cat(i,"TRUE","\n")
...
write.csv(df_merge, file="data_nuts2_wide.csv")
