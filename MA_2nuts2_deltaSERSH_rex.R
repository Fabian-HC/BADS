...
# A_fit1_1900 <- lm(dSERSH ~ trade_center + ...,
#                   data = subset(df, Year==1900))
# summary(A_fit1_1900)
# ...per Panel, Fit, and Year
dt <- df %>% group_by(nuts_2) %>% mutate(dSERSH = (SERSH-lag(SERSH))/delta_Year)
df$dSERSH <- dt$dSERSH
# === FIT 1 === #
# Controls:
...
A_fit1 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ trade_center +
...
B_fit1 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ ln_tc_dist +
                         country + latitude + longitude + altitude +
                         dist_border_ln + dist_coast_ln,
                       data = data)))
...
C_fit1 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ trade_years +
                         country + latitude + longitude + altitude +
                         dist_border_ln + dist_coast_ln,
                       data = data)))
...
D_fit1 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ tc_ratio +
                         country + latitude + longitude + altitude +
                         dist_border_ln + dist_coast_ln,
                       data = data)))
...
E_fit1 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ tc_dens +
                         country + latitude + longitude + altitude +
                         dist_border_ln + dist_coast_ln,
                       data = data)))
...
# PANEL A: trade center dummy
A_fit2 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ trade_center +
                         country + latitude + longitude + altitude +
...
# PANEL B: distance to trade
B_fit2 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ ln_tc_dist +
                         country + latitude + longitude + altitude +
                         capital + mountain + mining + AREA_ln + agri_ln,
                       data = data)))
...
# PANEL C: centuries of trade 
C_fit2 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ trade_years +
                         country + latitude + longitude + altitude +
                         capital + mountain + mining + AREA_ln + agri_ln,
                       data = data)))
...
# PANEL D: share NUTS3 trade regions per NUTS2 region
D_fit2 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ tc_ratio +
                         country + latitude + longitude + altitude +
                         capital + mountain + mining + AREA_ln + agri_ln,
                       data = data)))
...
# PANEL E: NUTS3 trade regions per NUTS2 area
E_fit2 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ tc_dens +
                         country + latitude + longitude + altitude +
                         dist_border_ln + dist_coast_ln,
                       data = data)))
...
# PANEL A: trade center dummy
A_fit3 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ trade_center +
                         country + latitude + longitude + altitude +
...
# PANEL B: distance to trade 
B_fit3 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ ln_tc_dist +
                         country + latitude + longitude + altitude +
                         university_1500 + printingpress_pre1500 + bishop + imperial + hanse + residence,
                       data = data)))
...
# PANEL C: centuries of trade
C_fit3 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ trade_years +
                         country + latitude + longitude + altitude +
                         university_1500 + printingpress_pre1500 + bishop + imperial + hanse + residence,
                       data = data)))
...
# PANEL D: share NUTS3 trade regions per NUTS2 region
D_fit3 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ tc_ratio +
                         country + latitude + longitude + altitude +
                         university_1500 + printingpress_pre1500 + bishop + imperial + hanse + residence,
                       data = data)))
...
# PANEL E: NUTS3 trade regions per NUTS2 area
E_fit3 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ tc_dens +
                         country + latitude + longitude + altitude +
                         dist_border_ln + dist_coast_ln,
                       data = data)))
...
# PANEL A: trade center dummy
A_fit4 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ trade_center +
                         country + latitude + longitude + altitude +
                         capital + mountain + mining + AREA_ln + agri_ln +
                         university_1500 + printingpress_pre1500 + bishop + imperial + hanse + residence,
...
# PANEL B: distance to trade 
B_fit4 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ ln_tc_dist +
                         country + latitude + longitude + altitude +
                         capital + mountain + mining + AREA_ln + agri_ln +
                         university_1500 + printingpress_pre1500 + bishop + imperial + hanse + residence,
...
# PANEL C: centuries of trade
C_fit4 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ trade_years +
                         country + latitude + longitude + altitude +
                         capital + mountain + mining + AREA_ln + agri_ln +
                         university_1500 + printingpress_pre1500 + bishop + imperial + hanse + residence,
...
# PANEL D: share NUTS3 trade regions per NUTS2 region
D_fit4 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ tc_ratio +
                         country + latitude + longitude + altitude +
                         dist_border_ln + dist_coast_ln,
                       data = data)))
...
# PANEL E: NUTS3 trade regions per NUTS2 area
E_fit4 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ tc_dens +
                         country + latitude + longitude + altitude +
                         dist_border_ln + dist_coast_ln,
                       data = data)))
...
# PANEL A: trade center dummy
A_fit5 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ trade_center +
...
# PANEL B: distance to trade 
B_fit5 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ ln_tc_dist +
                         nuts_1 + latitude + longitude + altitude +
                         capital + mountain + mining + AREA_ln + agri_ln +
                         university_1500 + printingpress_pre1500 + bishop + imperial + hanse + residence,
...
# PANEL C: centuries of trade
C_fit5 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ trade_years +
                         nuts_1 + latitude + longitude + altitude +
                         capital + mountain + mining + AREA_ln + agri_ln +
                         university_1500 + printingpress_pre1500 + bishop + imperial + hanse + residence,
...
# PANEL D: share NUTS3 trade regions per NUTS2 region
D_fit5 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ tc_ratio +
                         country + latitude + longitude + altitude +
                         dist_border_ln + dist_coast_ln,
                       data = data)))
...
# PANEL E: NUTS3 trade regions per NUTS2 area
E_fit5 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ tc_dens +
                         country + latitude + longitude + altitude +
                         dist_border_ln + dist_coast_ln,
                       data = data)))
...
# PANEL A: trade center dummy
A_fit6 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ trade_center +
                         country,
                       data = data)))
...
# PANEL B: distance to trade 
B_fit6 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ ln_tc_dist +
                         country,
                       data = data)))
...
# PANEL C: centuries of trade 
C_fit6 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ trade_years +
                         country,
                       data = data)))
...
# PANEL D: share NUTS3 trade regions per NUTS2 region
D_fit6 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ tc_ratio +
                         country,
                       data = data)))
...
# PANEL E: NUTS3 trade regions per NUTS2 area
E_fit6 <- df %>% filter(Year>1900) %>% nest_by(Year) %>%
  mutate(mod = list(lm(dSERSH ~ tc_dens +
                         country,
                       data = data)))
